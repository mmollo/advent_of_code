defmodule A do
  ...
  def turn_on(a, b, acc) do
    get_ranges(a, b)
    |> Enum.reduce(acc, fn x, acc -> Map.put(acc, x, true) end)
  end

  defp turn_off(a, b, acc) do
    get_ranges(a, b)
    |> Enum.reduce(acc, fn x, acc -> Map.put(acc, x, false) end)
  end

  def toggle(a, b, acc) do
    get_ranges(a, b)
    |> Enum.reduce(acc, fn x, acc -> Map.update(acc, x, true, fn z -> !z end) end)
  end
